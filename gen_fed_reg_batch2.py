...
# Fetch Federal Register documents for Feb 7 with different page
url = "https://www.federalregister.gov/api/v1/documents?publication_date=2026-02-07&per_page=100&page=2"

try:
    with urllib.request.urlopen(url) as response:
        data = json.loads(response.read().decode())
    
    selected_docs = data.get('results', [])[:50]
    print(f"Fetched {len(selected_docs)} documents for stories 263-{262+len(selected_docs)}", file=sys.stderr)
...
<p>Government Policy | {datetime.now().strftime('%B %d, %Y')}</p>
    ...
    print(f"✓ Generated {filename}: {title[:40]}", file=sys.stderr)
